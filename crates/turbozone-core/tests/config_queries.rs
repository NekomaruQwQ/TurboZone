...
fn verification_preserves_all_authored_values_on_success_and_failure() {
    ...
    config.rules[1].resize = ResizeRule::Exact { exact: [0, 900] };
...
fn resize_queries_interpret_every_form_without_changing_the_variant() {
    for (resize, primary, selector) in [
        ...
        ("resize.exact = [1440, 900]", Some([1440, 900]), None),
        ("resize = [1440, 900]", Some([1440, 900]), Some(ResizeSelector {
            default: Some([1440, 900]), ..Default::default()
        })),
